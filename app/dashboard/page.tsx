...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      // ✅ قراءة من Supabase أولاً (الأولوية)
      try {
        ...
        if (response.ok) {
          ...
          if (result.success && result.data && result.data.length > 0) {
            ...
            setLoading(false);
            return; // ✅ نجحنا — لا نحتاج LocalStorage
          }
        }
      } catch (err) {
      ...
      // ❌ Fallback: LocalStorage (إذا فشل Supabase)
